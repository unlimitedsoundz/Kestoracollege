...
async function main() {
    ...
    for (let i = 0; i < newHeads.length; i++) {
        ...
        const email = `${names[0].replace('.', '').toLowerCase()}.${lastName}@kestora.fi`; // Construct simple email
        ...
        const facultyData = {
            ...
            email: email,
            role: role,
            bio: `${role} at Kestora College. Expert in their field with a focus on sustainable development and academic excellence.`,
